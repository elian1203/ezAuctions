...
public abstract class SQLStorage extends DataSource {
	...
	protected boolean runVersioning() {
		try (Connection con = getConnection()) {
			...
			Bukkit.getLogger().log(Level.SEVERE, "[ezAuctions] Error updating database", e);
			return false;
		}
	}
		...
		runAsync(() -> {
			...
			try (Connection connection = getConnection()) {
				...
				Bukkit.getLogger().log(Level.SEVERE,
						"[ezAuctions] Error updating SQL player data for player " + player, ex);
			} finally {
				unlock();
			}
		});
	...
	public void updateIgnored(List<AuctionsPlayer> list, final AuctionsPlayer player) {
		runAsync(() -> {
			...
			try (Connection connection = getConnection()) {
				...
				Bukkit.getLogger().log(Level.SEVERE,
						"[ezAuctions] Error updating SQL ignored player data for player " + player, ex);
			} finally {
				unlock();
			}
		});
	...
	public void updateItems(List<AuctionsPlayer> list, final AuctionsPlayer player) {
		runAsync(() -> {
			...
			try (Connection connection = getConnection()) {
				...
				Bukkit.getLogger().log(Level.SEVERE,
						"[ezAuctions] Error updating SQL player items data for player " + player, ex);
			} finally {
				unlock();
			}
		});
